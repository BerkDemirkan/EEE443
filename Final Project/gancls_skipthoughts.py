...
path_to_umodel = f"{path_to_models}uni_skip.npz"
path_to_bmodel = f"{path_to_models}bi_skip.npz"
...
def load_tables():
    ...
    """
    words = []
    utable = numpy.load(f"{path_to_tables}utable.npy")
    btable = numpy.load(f"{path_to_tables}btable.npy")
    with open(f"{path_to_tables}dictionary.txt", "rb") as f:
...
def build_encoder_bi(tparams, options):
    ...
    projr = get_layer(options["encoder"])[1](
...
def gru_layer(tparams, state_below, options, prefix="gru", mask=None, **kwargs):
    ...
    state_belowx = (
        tensor.dot(state_below, tparams[_p(prefix, "Wx")]) + tparams[_p(prefix, "bx")]
    )
    ...
    rval, updates = theano.scan(
        _step,
        sequences=seqs,
        outputs_info=[tensor.alloc(0.0, n_samples, dim)],
