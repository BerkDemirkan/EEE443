...
class GAN:
    ...
    def build_model(self):
        ...
        t_real_caption = tf.placeholder(
            "float32",
            [self.options["batch_size"], self.options["caption_vector_length"]],
    ...
    def discriminator(self, image, t_text_embedding, reuse=False):
        ...
        h1 = ops.lrelu(
            self.d_bn1(ops.conv2d(h0, self.options["df_dim"] * 2, name="d_h1_conv"))
        )  # 16
        h2 = ops.lrelu(
            self.d_bn2(ops.conv2d(h1, self.options["df_dim"] * 4, name="d_h2_conv"))
        )  # 8
        h3 = ops.lrelu(
            self.d_bn3(ops.conv2d(h2, self.options["df_dim"] * 8, name="d_h3_conv"))
        )  # 4
